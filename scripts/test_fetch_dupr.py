...
def test_missing_players_are_fetched_with_exclude(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(fetch_dupr, "get", lambda token, path: {"id": 99} if "profile" in path else {"id": 99, "x": 1})
    ...
    fetch_dupr.main()
    assert [p["id"] for p in json.loads((tmp_path / "joueurs.json").read_text())] == ids + [99]
